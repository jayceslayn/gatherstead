...
- **Authorization refinement**: The remaining gap is HTTP status differentiation at the **service tier**. The attribute tier already distinguishes correctly (`RequireTenantAccessAttribute`/`RequireAppAdminAttribute` return 401 `UnauthorizedObjectResult` when unauthenticated, 403 `ForbidResult` when authenticated with an insufficient role), but service-layer guards attach a `PERMISSION_*` `ErrorCode` that every controller then returns as **400 Bad Request** — conflating "malformed request" with "not allowed". Next step: map the eight `PERMISSION_*` codes to 403 in a shared controller result helper (the existing `AccountDeletionActionResult` is the precedent for status-aware mapping). Role-management endpoints (`TenantUsersController`/`HouseholdUsersController`), non-escalation (`ServiceGuards.RequireNonEscalatingRole`), and last-owner protection (`TenantUserService.RequireOtherOwnerExistsAsync`) have all shipped.
