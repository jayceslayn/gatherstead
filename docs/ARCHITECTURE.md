...
- Validate inputs (model validation attributes/FluentValidation), enforce authorization per-tenant and per-resource, and log audit events for sensitive operations. Resource-level authorization (Self, Household Admin) is enforced at the service layer via `IMemberAuthorizationService`, while tenant-level authorization remains in the `RequireTenantAccessAttribute` filter.
