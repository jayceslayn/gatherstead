...
- **Accommodation**: A place a member may occupy (e.g., guest room, bunk, RV pad, tent site, or offsite placeholder). Exists independently of any event — the room inventory doesn't change per gathering. Captures type, a structured **bed inventory** (`AccommodationBed`: `BedSize` + quantity), room/spot dimensions (`WidthMeters`/`DepthMeters` with an optional `AreaSqMeters` override — persisted in metres, converted to feet/area for display), and notes. A "sleeps" capacity is derived from the bed inventory via `BedSizes.Sleeps`.【F:src/Gatherstead.Data/Entities/Accommodation.cs】
- **AccommodationBed**: One line of an accommodation's bed inventory (`BedSize` + `Quantity`). Embedded/full-replace on the parent accommodation (mirroring the attribute pattern via `BedSyncHelper`); unique per `(TenantId, AccommodationId, Size)`.【F:src/Gatherstead.Data/Entities/AccommodationBed.cs】
- **AccommodationIntent**: Member's request to occupy an accommodation on a given night, with a single lifecycle `Status` (`Requested`/`Hold`/`Confirmed`/`Declined`) and an informational party headcount (`PartyAdults`/`PartyChildren`). Not scoped to an event; nights may fall outside any formal gathering. Availability sums the party size of overlapping non-`Declined` stays against the accommodation's sleeps capacity.【F:src/Gatherstead.Data/Entities/AccommodationIntent.cs】
...
- **EventAttendance**: Per-member, per-day attendance record. Tracks `AttendanceStatus` and notes; drives meal and task intent generation for the member.【F:src/Gatherstead.Data/Entities/EventAttendance.cs】
...
- **MealIntent**: Member's cook sign-up for a `MealPlan`. The row's existence is the sign-up (withdrawal deletes it); `Source: IntentSource` records who initiated it (`Volunteered` = self/household-manager, `Assigned` = coordinator). Parallel in shape to `TaskIntent`.【F:src/Gatherstead.Data/Entities/MealIntent.cs】
...
- **TaskIntent**: Member's sign-up for a `TaskPlan`. Row existence = signed up (withdrawal deletes it); `Source: IntentSource` records self-signup (`Volunteered`) vs. coordinator assignment (`Assigned`). `Source` is server-derived from the acting user, never client-supplied.【F:src/Gatherstead.Data/Entities/TaskIntent.cs】
...
- **API contract pattern**: Each domain's wire types (Dto record, Response wrapper, CreateRequest, UpdateRequest) are co-located in a single `*Contracts.cs` file per domain (e.g., `AddressContracts.cs`). This is the canonical pattern — do not split request/response types into individual files. `AddControllers()` registers a global `JsonStringEnumConverter` so non-flags enums serialize as strings in both API responses and the OpenAPI spec. `[Flags]` enums (`MealTypeFlags`, `TaskTimeSlotFlags`) override the global converter with `[JsonConverter(typeof(JsonNumberEnumConverter<T>))]` on the specific record parameter/property so they remain integers.
- Favor Entity Framework Core migrations for schema changes; keep migrations deterministic and seed data idempotent. **Raw `migrationBuilder.Sql` must be wrapped in `EXEC(N'…')`** (double any single quotes inside). An `--idempotent` script emits raw SQL verbatim inside its `IF NOT EXISTS(… __EFMigrationsHistory …)` guard, and SQL Server binds column names when it compiles the batch even when the guard is false — so a data-migration statement that reads a column its own migration later drops makes the whole script unparseable once applied. `EXEC` defers it to a nested batch. A test in `Gatherstead.Data.Tests` enforces this.
...
- **Error status mapping**: Authorization is enforced at two tiers, and each maps its own failures. The attribute tier (`RequireTenantAccessAttribute`, `RequireAppAdminAttribute`) returns 401 when the caller is unauthenticated and 403 when authenticated with an insufficient role. The service tier raises denials as a `PERMISSION_*` `ErrorCode` on the response; controllers convert a failed response with `this.ToErrorResult(response)` (`ServiceErrorActionResult`), which returns **403 for any `PERMISSION_*` error and 400 otherwise**. The permission codes are matched by their enum-name prefix, so a new `PERMISSION_*` member maps to 403 without registration. Both paths keep the response body — the frontend localizes by `ErrorCode` and never sniffs HTTP statuses — which is why this is not `ControllerBase.Forbid()` (empty body, auth-handler challenge). Use `BadRequest(...)` directly only for malformed input that never reaches a service (e.g. an unparseable id segment).
...
- **JIT provisioning & invitation claiming**: `POST /api/me/bootstrap` (`UserProvisioningService`, authenticated but not tenant-scoped) upserts the caller's internal `User` from the `sub` claim, refreshes `Email`, and auto-claims pending `Invitation`s matching the token's trusted email — trusted on the basis of the validated issuer, resolved from the `email` claim (or an email-shaped `preferred_username` fallback) and used for matching unless the IdP explicitly marks it unverified (an `email_verified`/`verified_email` claim present and equal to `false`). `HttpContextCurrentUserContext.CacheKey` is `public` so the service can pre-seed a brand-new user's id into `HttpContext.Items`, letting the auditing interceptor stamp the self-created row. `MembershipGrant.GrantAsync` is the shared, idempotent helper that both the immediate-accept (existing user) and bootstrap-claim paths use to add `TenantUser`/`HouseholdUser` rows without downgrading existing membership. The Nuxt `tenant.global.ts` middleware invokes bootstrap once before resolving tenants so an invited user's membership exists on first load.
- **Auth caching**: Hot auth data is cached across requests behind `IAuthCache` (`src/Gatherstead.Api/Security/AuthCache.cs`) — a stateless singleton wrapping `HybridCache` that owns every auth cache key and TTL in one place (bound from the `AuthCache` config section). Cached datasets: the token-revocation check (`auth:revoked:{jti}`), the `ExternalId → UserId` mapping, `User.IsAppAdmin`, and `TenantUser`/`HouseholdUser` roles. Only identifiers and role enums are cached — never Always-Encrypted PII — and role keys carry the `TenantId` so nothing crosses tenant boundaries. Callers pass the DB-query factory in, so it executes in their own scoped `GathersteadDbContext`. A "not found" user mapping is deliberately never cached, so a user provisioned moments later by bootstrap isn't shadowed by a sticky null; `InvalidateAllForUserAsync` evicts every user-keyed entry (mapping, admin flag, per-tenant roles) on account erasure, and role/revocation mutations evict their own keys. Each call site keeps its per-request `HttpContext.Items` cache in front of this layer. **L1 (in-process) only today**: registering an `IDistributedCache` (e.g. Azure Cache for Redis) before `AddHybridCache` promotes it to an L1+L2 backplane with no code change — cached values are plain records that round-trip through System.Text.Json. Until then, revocation trades up to the configured TTL of cross-instance propagation delay for eliminating a DB round-trip on every authenticated request.
- **Cross-tenant write backstop**: `AuditingSaveChangesInterceptor.ValidateTenantId` runs on every `SaveChanges` where a tenant context is present, as defense-in-depth behind the global query filters (never a replacement for them). An **Added** entity's `TenantId` must equal the current request's tenant; a **Modified** entity's `TenantId` must not differ from the value it was loaded with (reassignment). Either violation logs at `Critical` and throws `CrossTenantWriteBlockedException` — an `InvalidOperationException` subtype carrying the entity type, both tenant ids, and a `CrossTenantWriteReason`. `ExceptionLoggingMiddleware` catches it, records a `SecurityEventType.CrossTenantWriteBlocked` security event **from a fresh DI scope**, and returns the standard 500. The fresh scope is essential: the request-scoped `DbContext` still holds the poisoned entity in its change tracker, so saving the event through it would re-trigger the interceptor and lose the event.
...
- **Demo site**: A zero-friction demo deployment uses the same codebase built as a static SPA (`NUXT_PUBLIC_DEMO_MODE=true pnpm generate`). A build-time `__DEMO_MODE__` constant (injected via `vite.define`) lets Rollup eliminate the inactive branch entirely — live builds carry no demo repository code and demo builds carry no live API client code. Repository classes are grouped in per-mode barrel files (`repositories/live/index.ts`, `repositories/demo/index.ts`); the plugin performs a single dynamic import per mode. See [DEMO_SITE.md](agents/plans/DEMO_SITE.md) for the repository abstraction, entity limits, seed data, infrastructure, and CI/CD details.
...
- **TypeScript type generation**: Frontend data types are auto-generated from the OpenAPI spec — never hand-maintained. Run `scripts/generate-openapi.sh` to: (1) build the API in Release, (2) emit `src/Gatherstead.Api/openapi.json` via the Swashbuckle CLI dotnet local tool (`.config/dotnet-tools.json`), (3) run `pnpm generate:types` (`openapi-typescript`) to produce `app/repositories/generated/api.d.ts`, and (4) run `tools/FlagsCodegen` to reflect the C# `[Flags]` enum bit values into `app/repositories/generated/flags.gen.ts`. Both generated files are committed so the frontend build never requires dotnet to be installed. `app/repositories/types.ts` is a pure barrel over the generated files — it defines no types of its own, using `OmitAudit<T>` derivations to strip audit fields from entity summary types where needed. `app/repositories/typeUtils.ts` holds the three flag utility functions (`mealTypesFromFlags`, `taskSlotsFromFlags`, `mealTypeFlagsToTaskSlotFlags`) that import from the generated files. CI enforces spec freshness via the `check-openapi-freshness` job, which re-generates and `diff`s against the committed `openapi.json`.
