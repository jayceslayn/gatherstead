...
public class TenantAttributeService : ITenantAttributeService
{
    private const string EntityDisplayName = "Tenant attribute";

    private readonly GathersteadDbContext _dbContext;
    private readonly ICurrentTenantContext _currentTenantContext;
    private readonly IMemberAuthorizationService _memberAuthorizationService;

    public TenantAttributeService(
    ...
    public async Task<BaseEntityResponse<IReadOnlyCollection<TenantAttributeDto>>> ListAsync(
        Guid tenantId,
        IEnumerable<Guid>? ids = null,
        CancellationToken cancellationToken = default)
    {
        var response = new BaseEntityResponse<IReadOnlyCollection<TenantAttributeDto>>();
        ...
        var query = _dbContext.TenantAttributes
            .AsNoTracking()
        ...
        return BaseEntityResponse<IReadOnlyCollection<TenantAttributeDto>>.SuccessfulResponse(visible);
    }

    public async Task<TenantAttributeResponse> GetAsync(
        Guid tenantId,
        Guid attributeId,
        CancellationToken cancellationToken = default)
    {
        var response = new TenantAttributeResponse();
        ...
        var attribute = await ServiceGuards.LoadOrNotFoundAsync(
            ...
            _dbContext.TenantAttributes
                .AsNoTracking()
                .Where(a => a.TenantId == tenantId && a.Id == attributeId),
            EntityDisplayName,
            cancellationToken);
    ...
    public async Task<TenantAttributeResponse> CreateAsync(
        Guid tenantId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!ServiceValidationHelper.ValidateTenantContext(tenantId, _currentTenantContext, response))
            return response;
        if (!ServiceGuards.RequireRequest(request, "create tenant attribute", response))
            return response;
        ...
        var duplicateExists = await _dbContext.TenantAttributes
            .AsNoTracking()
            .AnyAsync(a => a.TenantId == tenantId && a.Key == normalizedKey, cancellationToken);

        if (duplicateExists)
        {
            response.AddResponseMessage(MessageType.ERROR, $"An attribute with key '{normalizedKey}' already exists for this tenant.");
            return response;
        }
        ...
        _dbContext.TenantAttributes.Add(attribute);
    ...
    public async Task<TenantAttributeResponse> UpdateAsync(
        Guid tenantId,
        Guid attributeId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!ServiceValidationHelper.ValidateTenantContext(tenantId, _currentTenantContext, response))
            return response;
        if (!ServiceGuards.RequireRequest(request, "update tenant attribute", response))
            return response;
        if (!await ServiceGuards.AuthorizeTenantManageAsync(response, _memberAuthorizationService, tenantId, cancellationToken))
            return response;
        ...
        if (!string.Equals(attribute.Key, normalizedKey, StringComparison.Ordinal))
        {
            var duplicateExists = await _dbContext.TenantAttributes
                .AsNoTracking()
                .AnyAsync(a => a.TenantId == tenantId && a.Key == normalizedKey && a.Id != attributeId, cancellationToken);
    ...
    public async Task<TenantAttributeResponse> DeleteAsync(
        Guid tenantId,
        Guid attributeId,
        CancellationToken cancellationToken = default)
    {
        ...
        GathersteadMetrics.RecordSoftDelete("TenantAttribute", tenantId);
        response.SetSuccess(MapToDto(attribute));
        return response;
    }
    ...
    private static TenantAttributeDto MapToDto(TenantAttribute attr) => new(
        attr.Id,
        attr.TenantId,
        attr.Key,
        attr.Value,
