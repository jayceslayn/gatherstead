...
/// <summary>Covers the per-plan attendance read (<see cref="MealAttendanceService.ListAsync"/>).</summary>
public class MealAttendanceServiceTests : IAsyncLifetime
{
    ...
    private MealAttendanceService CreateService() =>
        new(_dbContext,
            Mock.Of<ICurrentTenantContext>(c => c.TenantId == _tenantId),
            Mock.Of<IMemberAuthorizationService>(),
            Mock.Of<IAuditVisibilityContext>());

    private async Task AddAttendanceAsync(Guid memberId)
    {
        _dbContext.MealAttendances.Add(new MealAttendanceEntity
        {
            Id = Guid.NewGuid(), TenantId = _tenantId, MealPlanId = _planId,
            HouseholdMemberId = memberId, Status = AttendanceStatus.Going,
        });
        await _dbContext.SaveChangesAsync(TestContext.Current.CancellationToken);
    }

    [Fact]
    public async Task ListAsync_ReturnsAttendancesForPlan()
    {
        ...
        await AddAttendanceAsync(_member);
    ...
    {
        await AddAttendanceAsync(_member2);
